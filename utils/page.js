...
export function getPageModules(id) {
  return getPage(id, true).then(page => page.fields.modules.map((module) => {
    if (module.sys.contentType.sys.id === 'productSlider') {
      return ProductSlider(module);
    }
    if (module.sys.contentType.sys.id === 'content') {
      return Content(module);
    }
    if (module.sys.contentType.sys.id === 'banner') {
